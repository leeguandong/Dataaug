...
class DataAugmentForObjectDetection():
    def __init__(self, change_light_rate=0.5, add_noise_rate=0.5, gaussianblur=0.5, random_point=0.5, flip_rate=0.5, shift_rate=0.5,
                 rand_point_percent=0.03, is_addNoise=False, is_changeLight=True, is_random_point=False, is_shift_pic_bboxes=True,
                 is_filp_pic_bboxes=True, is_addGaussianBlur=False):
        # 配置各个操作的属性
        ...
        self.add_gaussianblur = gaussianblur
        ...
        self.is_addGaussianBlur = is_addGaussianBlur

    # 加噪声
    def _addNoise(self, img):
        return random_noise(img, mode='salt', seed=int(time.time())) * 255
    ...
    def _changeLight(self, img):
        ...
        return cv2.addWeighted(img, alpha, blank, 1 - alpha, g)
    ...
    def dataAugment(self, img, dic_info):
        ...
        while change_num < 1:  # 默认至少有一种数据增强生效
            ...
            if self.is_addGaussianBlur:
                if random.random() < self.add_gaussianblur:  # 加随机点
                    change_num += 1
                    img = self._addGaussianBlur(img)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source_img_json_path', type=str, default='E:/qiantupsd/viewable_area/png_copy')
    parser.add_argument('--save_img_json_path', type=str, default='E:/qiantupsd/viewable_area/mcbv1_common')
